...
export const login = async (event: H3Event) => {
    ...
    const isPasswordCorrect = await comparePassword(body.password, user.hashPassword)
    ...
    setCookie(event, "token", token, { httpOnly: true, secure: true, sameSite: "strict" })
...
export const logout = async (event: H3Event) => {
    ...
    setCookie(event, "token", "", { httpOnly: true, secure: true, sameSite: "strict", maxAge: 0 })
